...
(def corpus
  [["ab" ["ab" "a" "abc" "" "ba"]]
   ...
   ;; a thread that STEPS ONTO a `$` was being told it was not at the end,
   ;; because the eol flag passed to the step was the one for the position the
   ;; threads left rather than the one they land on. `^a$` was right (the
   ;; anchor is reached before any step) and every longer anchored pattern was
   ;; wrong -- `^(px|em)$` answered false for "px". 216 checks agreed with
   ;; JavaScript without touching it.
   ["^a$" ["a" "b" "aa" ""]]
   ["^(px|em)$" ["px" "em" "pxem" "p" ""]]
   ["^[-+]?[0-9]+$" ["12" "-12" "+3" "1a" ""]]
   ["[0-9]+$" ["12" "a12" "12a" ""]]
